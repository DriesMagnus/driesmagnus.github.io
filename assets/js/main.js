...
function showYearAndHighlight(year, rowIndexes) {
  showYear(year);
  ...
  rowIndexes.forEach((index) => {
    const row = document.querySelector(
      `#table-${year} tbody tr:nth-child(${index})`
    );
    if (row) {
      const originalColor = window.getComputedStyle(row).backgroundColor;

      row.classList.add("highlighted");

      // Remove the highlight after the fade-out effect
      setTimeout(() => {
        row.style.transition = "background-color 1s ease-in-out";
        row.style.backgroundColor = originalColor;
        setTimeout(() => {
          row.style.backgroundColor = ""; // Reset to original style (works for both transparent and custom colors)
